...
bool deserialize(IoBuffer& buffer, TwoEncoderStatus& status)
{
    ...
    status.left_direction_pin_state = j[0]["ps"].as<int8_t>();
    ...
    status.right_direction_pin_state = j[1]["ps"].as<int8_t>();
    return true;
}
...
bool deserialize(IoBuffer& buffer, InputMessage& inmsg)
{
    std::vector<std::function<bool(IoBuffer&)>> pv = {
        [&](IoBuffer& buffer){
            if(CmdResponse msg; deserialize(buffer, msg)) {
                inmsg = msg;
                return true;
            }
            return false;
...
{
    buffer.clear();
    auto len = snprintf((char*)buffer.space_ptr(), buffer.space_len(), "%s %s", (response.ok.c_str() ? "OK": "FAILED"), response.text.c_str());
    buffer.commit(len);
