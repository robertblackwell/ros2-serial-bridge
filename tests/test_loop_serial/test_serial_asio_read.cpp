...
struct ReadLoop {
    serial_bridge::SerialAsio& m_serial;
    ReadLoop(serial_bridge::SerialAsio& serial):m_serial(serial){}
    ~ReadLoop(){}
    void run(std::function<void(IoBuffer::UPtr up, boost::system::error_code& ec)> cb)
    {
        runner(cb);
    }
    void runner(std::function<void(IoBuffer::UPtr up, boost::system::error_code& ec)> cb)
    {
        // auto self(boost::asio::shared_from_this());
        m_serial.recv([this, cb](IoBuffer::UPtr up, boost::system::error_code& ec)
        {
            if (!ec) {
                // std::cout << "do_read got [" << up->c_str() << "]" << "\n";
                cb(std::move(up), ec);
                runner(cb);
            } else {
                std::cerr << ec.message() << "\n";
...
void* reader_thread_01(const std::string& dev)
{
    ...
    std::chrono::time_point<std::chrono::high_resolution_clock>* end_time_ref = &end_time_g;
     // ReadLoop read_loop(serial);
     // read_loop.run([](IoBuffer::UPtr up, boost::system::error_code& ec)
     serial.run([](IoBuffer::UPtr up, boost::system::error_code ec)
     {
         if (!ec) {
...
//     auto do_read = [&serial]()
//     {
//         serial.recv([](IoBuffer::UPtr up, boost::system::error_code ec)
//         {
//             if (ec) {
//                 std::cout << ec.message() << "\n";
//             } else {
...
//     }
// #endif
    // serial.run([&index, max_index, end_time_ref, &char_count](IoBuffer::UPtr up, boost::system::error_code& ec)
    // {
    //
